...
public class RezervationsFragment extends Fragment {
    ...
    private void loadData() {
        ...
        Call<List<RezervationResponse>> call = apiService.getRezervations(userId); // Rezervasyonları getirme isteği oluştur
        call.enqueue(new Callback<List<RezervationResponse>>() { // İstek başarılıysa veya başarısızsa geri çağırma işlevi ayarla
            @Override
            public void onResponse(Call<List<RezervationResponse>> call, Response<List<RezervationResponse>> response) {
                if (response.isSuccessful() && response.body() != null) { // İstek başarılı ve boş değilse
                    ArrayList<RezervationResponse> reservationList = new ArrayList<>(response.body()); // Yanıttan rezervasyon listesini al
            ...
            @Override
            public void onFailure(Call<List<RezervationResponse>> call, Throwable t) { // İstek başarısız olduğunda
